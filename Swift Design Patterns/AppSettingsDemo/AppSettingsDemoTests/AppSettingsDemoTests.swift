...
class AppSettingsDemoTests: XCTestCase {
    ...
    func testConcurrentUsage(){
        ...
        let expect = expectation(description: "Using appsetngs from multipe")
        
        let callCount = 100
        
        for callIndex in 1...callCount{
            concurrentQueue.async {
                AppSettings.shared.setValueForKey(value: callIndex, forKey: String(callIndex))
            }
        }
